...
from ontology.code.vmtoken import VMTokenizer
from ontology.code.expression import Expression
from ontology.code import pyop
from ontology.code.ast_preprocess import preprocess_method_body, ABI
from uuid import uuid4
from ontology.code.pytoken import PyToken
...
class method(object):
    ...
    @property
    def is_interop(self):
        if 'ontology.interop' in self.full_name:
            return True
        if 'ontology.builtins' in self.full_name and self.full_name != 'ontology.builtins.range':
            return True
        return False
